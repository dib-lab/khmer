...
int main( int argc, char * argv[ ] )
{
    ...
    if (optind < argc) ifile_name_1 = string( argv[ optind++ ] );
    else error( EINVAL, 0, "Name of first hash table file required" );

    if (optind < argc) ifile_name_2 = string( argv[ optind++ ] );
    else error( EINVAL, 0, "Name of second hash table file required" );
    
    CountingHash ht1( 20, 1 );	CountingHash ht2( 20, 1 );
    printf( "Loading hash tables into memory....\n" );
    ht1.load( ifile_name_1 );	ht2.load( ifile_name_2 );
