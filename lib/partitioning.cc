                                            ...
                                            uint64_t& n_consumed)
{
    ...
    while(!kmers.done()) {
        ...
        if (is_new_kmer) {
            ...
            search_from.push_back(kmer);
            in_known_territory = false;
            foud_tag_in_territory = false;
            ++since;
        } else {
    ...
    Component * comp;
    for (auto tag: tags) {
        if ((comp = tag_component_map.get(kmer)) != NULL) {
            comps.insert(comp);
        }
    }
                                               ...
                                               std::set<HashIntoType>& seen)
{
    ...
    const unsigned int max_breadth = (2 * graph->_tag_density) + 1;
