...
pkgboost::mpl::aux::weighted_tag<1>::type operator==( assert_, assert_ );
pkgboost::mpl::aux::weighted_tag<2>::type operator!=( assert_, assert_ );
pkgboost::mpl::aux::weighted_tag<3>::type operator>(  assert_, assert_ );
pkgboost::mpl::aux::weighted_tag<4>::type operator>=( assert_, assert_ );
pkgboost::mpl::aux::weighted_tag<5>::type operator<( assert_, assert_ );
pkgboost::mpl::aux::weighted_tag<6>::type operator<=( assert_, assert_ );
...
struct eval_assert {
    ...
    typedef typename ::pkgboost::mpl::if_c<p_type::value,
...
template< typename Pred >
typename assert_arg_type< pkgboost::mpl::not_<Pred> >::type 
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          pkgboost::mpl::assertion_failed<false>( \
              pkgboost::mpl::make_assert_arg< \
                  typename pkgboost::mpl::eval_assert<void pred>::type \
                >() \
            ) \
        ) \
    ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          pkgboost::mpl::assertion_failed<false>( \
              pkgboost::mpl::make_assert_arg< \
                  typename pkgboost::mpl::eval_assert_not<void pred>::type \
                >() \
            ) \
        ) \
    ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          pkgboost::mpl::assertion_failed<false>( \
              pkgboost::mpl::assert_arg( (void (*) pred)0, 1 ) \
            ) \
        ) \
    ) \
/**/
...
enum { \
      BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
          pkgboost::mpl::assertion<false>::failed( \
              pkgboost::mpl::assert_not_arg( (void (*) pred)0, 1 ) \
            ) \
        ) \
}\
/**/
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        pkgboost::mpl::assertion_failed<BOOST_PP_CAT(mpl_assert_rel_value,counter)>( \
            (pkgboost::mpl::failed ************ ( pkgboost::mpl::assert_relation< \
                  pkgboost::mpl::assert_::relations( sizeof( \
                      pkgboost::mpl::assert_::arg rel pkgboost::mpl::assert_::arg \
                    ) ) \
                , x \
                , y \
                >::************)) 0 ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
    ...
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        pkgboost::mpl::assertion_failed<BOOST_PP_CAT(mpl_assert_rel_value,counter)>( \
              pkgboost::mpl::assert_rel_arg( pkgboost::mpl::assert_relation< \
                  pkgboost::mpl::assert_::relations(BOOST_PP_CAT(mpl_assert_rel,counter)) \
                , x \
                , y \
                >() ) \
            ) \
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
        pkgboost::mpl::assertion_failed<(x rel y)>( pkgboost::mpl::assert_rel_arg( \
              pkgboost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(x,y,(&pkgboost::mpl::operator rel))() \
            ) ) \
        ) \
    ) \
/**/
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,BOOST_MPL_AUX_PP_COUNTER()) = sizeof( \
        pkgboost::mpl::assertion_failed<(x rel y)>( (pkgboost::mpl::failed ************ ( \
            pkgboost::mpl::BOOST_MPL_AUX_ASSERT_RELATION(x,y,(&pkgboost::mpl::operator rel))::************))0 ) \
        ) \
    ) \
/**/
#   endif
...
struct msg; \
typedef struct BOOST_PP_CAT(msg,counter) : pkgboost::mpl::assert_ \
{ \
    using pkgboost::mpl::assert_::types; \
    static pkgboost::mpl::failed ************ (msg::************ assert_arg()) types_ \
    { return 0; } \
} BOOST_PP_CAT(mpl_assert_arg,counter); \
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        pkgboost::mpl::assertion<(c)>::failed( BOOST_PP_CAT(mpl_assert_arg,counter)::assert_arg() ) \
        ) \
    ) \
/**/
#else
...
BOOST_MPL_AUX_ASSERT_CONSTANT( \
      std::size_t \
    , BOOST_PP_CAT(mpl_assertion_in_line_,counter) = sizeof( \
        pkgboost::mpl::assertion_failed<(c)>( BOOST_PP_CAT(mpl_assert_arg,counter)::assert_arg() ) \
        ) \
    ) \
/**/
#endif
