...
{
public:
    typedef pkgboost::uint16_t tag_t;
    typedef pkgboost::uint16_t index_t;
...
struct compiletime_sized_freelist_storage
{
    ...
    pkgboost::array<char, size * sizeof(T)> data;
    ...
    Alloc
{
    ...
    {
        if (count > 65535)
            pkgboost::throw_exception(std::runtime_error("boost.lockfree: freelist size is limited to a maximum of 65535 objects"));
