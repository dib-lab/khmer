...
#define BOOST_TYPEOF_modifier_support(ID, Fun)\
    ...
    {\
        typedef\
            typename pkgboost::type_of::encode_type<\
            typename pkgboost::type_of::push_back<\
            V\
            , pkgboost::mpl::size_t<ID> >::type\
            , T>::type\
            type;\
    };\
    template<class Iter> struct decode_type_impl<pkgboost::mpl::size_t<ID>, Iter>\
    {\
        typedef pkgboost::type_of::decode_type<Iter> d1;\
...
#define BOOST_TYPEOF_array_support(ID, Qualifier)\
    ...
    {\
        typedef\
            ...
            V\
            , pkgboost::mpl::size_t<ID> >::type\
            , pkgboost::mpl::size_t<N> >::type\
            , T>::type\
        type;\
    };\
    template<class Iter>\
    struct decode_type_impl<pkgboost::mpl::size_t<ID>, Iter>\
    {\
        enum{n = Iter::type::value};\
        typedef pkgboost::type_of::decode_type<typename Iter::next> d;\
