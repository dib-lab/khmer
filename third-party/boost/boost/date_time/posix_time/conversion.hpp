...
namespace posix_time {
  ...
  inline
  std::tm to_tm(const pkgboost::posix_time::ptime& t) {
    std::tm timetm = pkgboost::gregorian::to_tm(t.date());
    pkgboost::posix_time::time_duration td = t.time_of_day();
  ...
  inline
  std::tm to_tm(const pkgboost::posix_time::time_duration& td) {
    std::tm timetm;
  ...
  ptime ptime_from_tm(const std::tm& timetm) {
    pkgboost::gregorian::date d = pkgboost::gregorian::date_from_tm(timetm);
   ...
   *       pkgboost::date_time::winapi::file_time type.
   */
  template< typename TimeT, typename FileTimeT >
  inline
  TimeT from_ftime(const FileTimeT& ft)
  {
    return pkgboost::date_time::time_from_ftime<TimeT>(ft);
  }

#endif // BOOST_HAS_FTIME

} } //namespace pkgboost::posix_time
