...
then
	CFLAGS="-pg -fprofile-arcs -ftest-coverage" python setup.py build_ext --debug --inplace --libraries gcov
else
	python setup.py build_ext
fi

pip install --quiet nosexcover
python setup.py nosetests --with-xcoverage --with-xunit --cover-package=khmer --cover-erase --attr=\!known_failing

make doc

pip install --quiet pylint
pylint -f parseable doc/*.py figuregen/*.py novelty/*.py khmer/*.py sandbox/*.py scripts/*.py tests khmer | tee ../pylint.out

cppcheck --std=posix --platform=unix64 -j8 --enable=all -I lib/ -i lib/zlib/ -i lib/bzip2/ -DVALIDATE_PARTITIONS --xml lib 2> cppcheck-result.xml
